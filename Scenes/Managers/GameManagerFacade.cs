...
namespace Scenes.Managers
{
	public class GameManagerFacade
	{
		...
		{
			// set state for round as weather modification
			var state = new HeroStateManager().GenerateRandomState();
			GD.Print($"BattleScene: Hero state for this round is: {state.GetStateName()}");
			MainHeroManager._hero.SetState(state);
