...
namespace Scenes.Managers
{
	public class MainHeroManager
	{
		...
		public MainHeroManager()
		{
			...
			_heroName = _hero.GetName();
		}

		public void SetNodes(Control heroDisplayContainer, Label moneyLabel, Label levelLabel)
		{
			if (heroDisplayContainer == null) throw new ArgumentNullException(nameof(heroDisplayContainer));
			_heroDisplayNode = GD.Load<PackedScene>(EnemyVisualScenePath).Instantiate<Control>();
			heroDisplayContainer.AddChild(_heroDisplayNode);
		...
		public void EarnRewards(int round)
		{
